use crate::{
    check_mutability, check_profile, chr_delete, chroot_exec, get_current_snapshot, get_tmp, grub,
    immutability_disable, immutability_enable, is_system_locked, is_system_pkg, post_transactions,
...
pub fn aur_check(snapshot: &str) -> bool {
    ...
    if options["aur"] == "True" {
        ...
        panic!(
            "Please insert valid value for aur in /.snapshots/etc/etc-{}/ash/ash.conf",
            snapshot
        );
    }
}
    ...
    if !Path::new(&format!("/.snapshots/rootfs/snapshot-{}", snapshot)).try_exists()? {
        ...
        if !aur_check(snapshot) {
            ...
            if excode.success() {
                ...
                return Err(Error::new(ErrorKind::Other, "Failed to upgrade."));
            }
        } else {
...
pub fn bootstrap(snapshot: &str) -> Result<(), Error> {
    ...
    if !excode.success() {
        return Err(Error::new(
            ErrorKind::Other,
            format!(
                "Failed to install base packages in snapshot {} chroot.",
                snapshot
            ),
        ));
    }
    ...
    let paru_install = Command::new("sh")
        .arg("-c")
        .arg(
            format!("su aur -c 'paru --dbpath {} -r /.snapshots/rootfs/snapshot-chr{} -Sy --noconfirm --overwrite \"*\" paru'", //TODO replace paru with ash
                     tmp_db.path().to_str().unwrap(),snapshot),
        )
        .status()?;
    if !paru_install.success() {
        return Err(Error::new(
            ErrorKind::Other,
            format!(
                "Failed to install paru package in snapshot {} chroot.",
                snapshot
            ),
        ));
    }
    ...
    remove_dir_content(&format!(
        "/.snapshots/rootfs/snapshot-chr{}/var/lib/pacman",
        snapshot
    ))?;
    Command::new("cp")
        .args(["-r", "--reflink=auto"])
...
pub fn cache_copy(snapshot: &str, prepare: bool) -> Result<(), Error> {
    ...
    if prepare {
        Command::new("cp")
            .args(["-n", "-r", "--reflink=auto"])
            .arg(format!(
                "/.snapshots/rootfs/snapshot-{}/var/cache/pacman/pkg/.",
                tmp
            ))
            .arg(format!(
                "/.snapshots/rootfs/snapshot-chr{}/var/cache/pacman/pkg",
                snapshot
            ))
            .output()
            .unwrap();
...
pub fn fix_package_db(snapshot: &str) -> Result<(), Error> {
        ...
        && !snapshot.is_empty()
    {
        return Err(Error::new(
            ErrorKind::NotFound,
            format!(
                "Cannot fix package man database as snapshot {} doesn't exist.",
                snapshot
            ),
        ));
    ...
    // Make sure snapshot is not in use
        ...
        return Err(Error::new(
            ...
            format!("Snapshot 0 (base) should not be modified."),
        ));
    } else if snapshot == "0" {
        // Base snapshot unsupported
        return Err(Error::new(
        ...
        run_chroot = if snapshot.is_empty() { false } else { true };
        ...
        if run_chroot {
            let etc_gnupg = format!(
                "/.snapshots/rootfs/snapshot-chr{}/etc/pacman.d/gnupg",
                snapshot
            );
            if Path::new(&etc_gnupg).try_exists()? && read_dir(&etc_gnupg)?.count() > 0 {
                cmds.push(format!("rm -rf /etc/pacman.d/gnupg"));
            }
            if Path::new(&format!("{}/.gnupg", home)).try_exists()?
                && read_dir(&format!("{}/.gnupg", home))?.count() > 0
            {
                cmds.push(format!("rm -rf {}/.gnupg", home));
            }
            if Path::new("/var/lib/pacman/sync").try_exists()?
                && read_dir("/var/lib/pacman/sync")?.count() > 0
            {
                cmds.push(format!("rm -r /var/lib/pacman/sync/*"));
            }
            cmds.push(format!("pacman -Syy"));
            cmds.push(format!(
                "sudo -u {} gpg --refresh-keys",
            ...
            if Path::new("/etc/pacman.d/gnupg").try_exists()?
                && read_dir("/etc/pacman.d/gnupg")?.count() > 0
            {
                cmds.push(format!("rm -rf /etc/pacman.d/gnupg"));
            }
            if Path::new(&format!("{}/.gnupg", home)).try_exists()?
        ...
        for cmd in cmds {
            if run_chroot {
                let excode = Command::new("sh")
                    .arg("-c")
                    .arg(format!(
                        "chroot /.snapshots/rootfs/snapshot-chr{} {}",
                        snapshot, &cmd
                    ))
                    .status()?;
                if !excode.success() {
                ...
                let excode = Command::new("sh").arg("-c").arg(&cmd).status()?;
                if !excode.success() {
                    return Err(Error::new(
                        ErrorKind::Other,
                        format!("Run command {} failed.", &cmd),
...
pub fn install_package_helper(
    ...
    let cfile = format!(
        "/.snapshots/rootfs/snapshot-chr{}/etc/ash/profile",
        snapshot
    );
    ...
    for pkg in pkgs {
        ...
        if profconf
            .sections()
            .contains(&"profile-packages".to_string())
        {
            for pkg in profconf
                .get_map()
                .unwrap()
        ...
        if excode.status.success() || excode_group.status.success() {
            ...
            return Err(Error::new(ErrorKind::NotFound, "Please enable AUR."));
        }
    }
    Ok(())
}
...
// Install atomic-operation
pub fn install_package_helper_chroot(
    snapshot: &str,
    pkgs: &Vec<String>,
    noconfirm: bool,
) -> Result<(), Error> {
...
pub fn install_package_helper_live(
    snapshot: &str,
    tmp: &str,
    pkgs: &Vec<String>,
    noconfirm: bool,
    ...
    for pkg in pkgs {
        // This extra pacman check is to avoid unwantedly triggering AUR if package is official
        let excode = Command::new("pacman")
            .arg("-Si")
            .arg(format!("{}", pkg))
            .output()?; // --sysroot
        let excode_group = Command::new("pacman")
            .arg("-Sg")
            .arg(format!("{}", pkg))
            .output()?;
        if excode.status.success() || excode_group.status.success() {
            ...
            let excode = Command::new("sh")
                .arg("-c")
                .arg(format!(
                    "chroot /.snapshots/rootfs/snapshot-{} {}",
                    tmp, pacman_args
                ))
                .status()?;
            if !excode.success() {
...
pub fn is_service_enabled(snapshot: &str, service: &str) -> bool {
    if Path::new("/var/lib/systemd/").try_exists().unwrap() {
        let excode = Command::new("sh")
            .arg("-c")
            .arg(format!(
                "chroot /.snapshots/rootfs/snapshot-chr{} systemctl is-enabled {}",
                snapshot, service
            ))
            .output()
            .unwrap();
...
pub fn lockpkg(snapshot: &str, profconf: &Ini) -> Result<(), Error> {
    // Open the file
    let pacman_conf_path = format!(
        "/.snapshots/rootfs/snapshot-chr{}/etc/pacman.conf",
        snapshot
    );
...
pub fn no_dep_pkg_list(snapshot: &str, chr: &str) -> Vec<String> {
    let excode = Command::new("sh")
        .arg("-c")
        .arg(format!(
            "chroot /.snapshots/rootfs/snapshot-{}{} pacman -Qqe",
            chr, snapshot
        ))
        .output()
        .unwrap();
...
pub fn pkg_list(snapshot: &str, chr: &str) -> Vec<String> {
    ...
    let excode = Command::new("sh")
        .arg("-c")
        .arg(format!(
            "chroot /.snapshots/rootfs/snapshot-{}{} pacman -Qq",
            chr, snapshot
        ))
        .output()
        .unwrap();
...
pub fn pkg_query(pkg: &str) -> Result<ExitStatus, Error> {
    let excode = Command::new("dpkg-query")
        .arg("-W")
...
pub fn refresh_helper(snapshot: &str) -> Result<(), Error> {
    let refresh = Command::new("chroot")
        .arg(format!("/.snapshots/rootfs/snapshot-chr{}", snapshot))
        .args(["pacman", "-Syy"])
        .status()?;
    // Avoid invalid or corrupted package (PGP signature) error
    let keyring = Command::new("chroot")
        .arg(format!("/.snapshots/rootfs/snapshot-chr{}", snapshot))
        .args(["pacman", "-S", "--noconfirm", "archlinux-keyring"])
        .status()?;
    if !refresh.success() {
        return Err(Error::new(ErrorKind::Other, "Refresh failed."));
    }
    if !keyring.success() {
        return Err(Error::new(
            ErrorKind::Other,
...
pub fn service_enable(snapshot: &str, services: &Vec<String>, chr: &str) -> Result<(), Error> {
    for service in services {
        ...
        if Path::new("/var/lib/systemd/").try_exists()? {
            ...
            if !excode.success() {
                // enable the service in live snapshot
                let excode_live = Command::new("systemctl")
                    .arg("enable")
                    .arg("--now")
                    .arg(&service)
                    .status()?;

                if !excode_live.success() {
                    return Err(Error::new(
                        ErrorKind::Other,
                        format!("Failed to enable {} in live snapshot.", service),
                    ));
                }
...
pub fn system_config(snapshot: &str, profconf: &Ini) -> Result<(), Error> {
    ...
    remove_dir_content(&format!(
        "/.snapshots/rootfs/snapshot-chr{}/etc/pacman.d",
        snapshot
    ))?;
    Command::new("cp")
        .args(["-n", "-r", "--reflink=auto"])
        .arg(format!(
            "/.snapshots/rootfs/snapshot-{}/etc/pacman.d/.",
            snapshot
        ))
        .arg(format!(
            "/.snapshots/rootfs/snapshot-chr{}/etc/pacman.d/",
    ...
    Command::new("cp")
        ...
        .arg(format!(
            "/.snapshots/rootfs/snapshot-chr{}/etc/ash",
            snapshot
        ))
        .output()?;
    ...
    Command::new("cp")
        .args(["-n", "-r", "--reflink=auto"])
        .arg(format!(
            "/.snapshots/rootfs/snapshot-{}/boot/{}/.",
            snapshot, grub
        ))
        .arg(format!(
            "/.snapshots/rootfs/snapshot-chr{}/boot/{}",
            snapshot, grub
        ))
        .output()?;
    #[cfg(feature = "grub")]
    Command::new("cp")
        .args(["-n", "-r", "--reflink=auto"])
        .arg(format!(
            "/.snapshots/rootfs/snapshot-{}/etc/default/grub",
            snapshot
        ))
        .arg(format!(
            "/.snapshots/rootfs/snapshot-chr{}/etc/default/grub",
            snapshot
        ))
        .output()?;
    #[cfg(feature = "grub")]
    Command::new("cp")
        .args(["-n", "-r", "--reflink=auto"])
        .arg(format!(
            "/.snapshots/rootfs/snapshot-{}/etc/grub.d",
            snapshot
        ))
        .arg(format!(
            "/.snapshots/rootfs/snapshot-chr{}/etc/grub.d",
            snapshot
        ))
        .output()?;
    ...
    // Copy /usr/local
    remove_dir_content(&format!(
        "/.snapshots/rootfs/snapshot-chr{}/usr/local",
        snapshot
    ))?;
    Command::new("cp")
        .args(["-n", "-r", "--reflink=auto"])
        .arg(format!(
            "/.snapshots/rootfs/snapshot-{}/usr/local/.",
            snapshot
        ))
        .arg(format!(
            "/.snapshots/rootfs/snapshot-chr{}/usr/local/",
    ...
    if profconf
        .sections()
        .contains(&"disable-services".to_string())
    {
        let mut services: Vec<String> = Vec::new();
        for service in profconf
            .get_map()
            .unwrap()
            .get("disable-services")
            .unwrap()
            .keys()
        {
    ...
    if profconf
        .sections()
        .contains(&"install-commands".to_string())
    {
        for cmd in profconf
            .get_map()
            .unwrap()
    ...
    if profconf
        .sections()
        .contains(&"system-configuration".to_string())
    {
        let mut system_conf: Vec<String> = Vec::new();
        for path in profconf
            .get_map()
            .unwrap()
...
pub fn uninstall_package_helper(
    snapshot: &str,
    pkgs: &Vec<String>,
    noconfirm: bool,
) -> Result<(), Error> {
...
// Uninstall package(s) atomic-operation
pub fn uninstall_package_helper_chroot(
    snapshot: &str,
    pkgs: &Vec<String>,
    noconfirm: bool,
) -> Result<(), Error> {
...
pub fn uninstall_package_helper_live(
    tmp: &str,
    pkgs: &Vec<String>,
    noconfirm: bool,
) -> Result<(), Error> {
...
pub fn upgrade_helper(snapshot: &str, noconfirm: bool) -> Result<(), Error> {
    ...
    if !aur_check(snapshot) {
        ...
        let excode = Command::new("sh")
            .arg("-c")
            .arg(format!(
                "chroot /.snapshots/rootfs/snapshot-chr{} su aur -c '{}'",
                snapshot, paru_args
            ))
            .status()
            .unwrap();
...
pub fn upgrade_helper_live(tmp: &str, noconfirm: bool) -> Result<(), Error> {
    ...
    if !aur_check(tmp) {
        ...
        let excode = Command::new("sh")
            .arg("-c")
            .arg(format!(
                "chroot /.snapshots/rootfs/snapshot-{} su aur -c '{}'",
                tmp, paru_args
            ))
            .status()
            .unwrap();
